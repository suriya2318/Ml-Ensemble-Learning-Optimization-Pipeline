...
[INSERT CHART 3 HERE — Cross-Validation F1-Score bar chart with error bars showing mean CV F1 and standard deviation for all 4 models. Lower error bar = more consistent performance. Tuned Random Forest bar is tallest with smallest error bar — best accuracy and most reliable. Place this chart in the Cross-Validation section immediately after the CV results table to visually confirm which model is most consistent across different patient data splits.]
...
[INSERT CHART 4 HERE — Feature Importance Horizontal Bar Chart showing top 15 most important features ranked from highest to lowest importance score. Each bar labeled with exact importance value. Top bar (worst_radius) highlighted in red, remaining bars in blue. Place this chart in the Feature Importance section immediately after the top features table. This chart is critical for model interpretability — it identifies which tumor measurements are most diagnostically significant for clinical trust.]
...
[INSERT CHART 1 HERE — Model Comparison Grouped Bar Chart showing F1-Score and ROC-AUC for all 4 models side by side. Blue = Logistic Regression, Teal = Random Forest, Amber = Gradient Boosting, Red = Tuned Random Forest. Y-axis starts at 0.93 to zoom into meaningful differences. Every bar labeled with exact score. Place this chart in the Final Model Comparison section immediately after the complete 4-model comparison table. This is the most comprehensive visualization in the entire report.]
[INSERT CHART 2 HERE — ROC Curves for all 4 models plotted on the same axes. Blue = Logistic Regression, Teal = Random Forest, Amber = Gradient Boosting, Red = Tuned Random Forest. Black dashed diagonal = random classifier (AUC = 0.50). AUC score for each model shown in legend. Place this chart in the Final Model Comparison section directly after Chart 1 to provide the threshold-independent view of each model's discrimination ability.]
Confusion Matrix — Best Model
[INSERT CHART 5 HERE — Confusion Matrix Heatmap for the Tuned Random Forest (red color scheme). 2×2 grid showing TN, FP, FN, TP counts with color intensity. Row labels = Actual class, Column labels = Predicted class. Place this chart in the Final Model Evaluation section immediately after the best model is declared and its test set metrics are printed. This confirms the actual error pattern of the winning model — the False Positive count (missed cancers) must be minimal for safe medical deployment.]
...
• Place in the Final Model Evaluation section after the best model is declared and test metrics reported.
...
Random Forest's bagging technique builds 100 independent trees on random subsets — individual tree errors cancel out when averaged by majority vote, producing a final prediction far more reliable than any single tree from Tasks 2 or 3.
